...
export default function Product({ Children, item = null, refechData = () => { } }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let response;
      if (item) {
        response = await axios.put(process.env.NEXT_PUBLIC_BASEURL + 'product/' + item?._id, payload, {
          headers: {
        ...
        response = await axios.post(process.env.NEXT_PUBLIC_BASEURL + 'product', payload, {
          headers: {
            "auth-token": authToken,
            'Content-Type': 'application/json',
          },
      ...
      <DialogContent>
        ...
        <form className="space-y-5">
          <div className="space-y-4">
            {[{ label: 'Name', name: 'name' }, { label: 'Price', name: 'price' }].map(({ label, name }) => (
              <div key={label} className="*:not-first:mt-2">
                ...
                <Input id={name}
                  ...
                  type={name==='price'?'number':"text"} required />
              </div>
            ))}
          </div>
          <Button
