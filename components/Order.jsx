...
export default function Order({ Children, item = null, refechData = () => { } }) {
  ...
  const [orderDetails, setOrderDetails] = useState({ phone: '', name: '', address: '' });
  ...
  const handleClear=()=>{
    if (item) {
      ...
      setOrderDetails({ phone: '', name: '', address: '' });
      setRemarks([{ reason: '' }])
    }
  } 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = { ...orderDetails, remark: remarks?.filter(obj => obj.reason?.trim()) };
      let response;
      if (item) {
